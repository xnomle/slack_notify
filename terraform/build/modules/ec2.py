...
def get_ec2_details(event):
    ...
    # 1. Grab standard details 
    account_number = event.get('account')
    region         = event.get('region')
    alert_time     = event.get('time')

    # 2. Grab details of event
    details = event.get('detail', {})
    
    # EventBridge 'resources' is a list of ARNs
    resources     = event.get("resources", [])
    # resource_arn  = resources[0] if resources else "Unknown" # Optional: if you want the full ARN
    ...
    try:
        ...
        # Get the reason, default to 'Unknown' if missing
        reason = instance_data.get('StateTransitionReason', 'Unknown')

        # 4. Construct Blocks with F-Strings
        message_blocks = [
            {
                "type": "header",
                "text": {
